...
function writeHistory(dir, fileName, entries) {
...
(function runTests() {
  ...
  (() => {
    ...
    try {
      ...
      threw = true;
      assert(/Telemetry file not found/.test(err.message), 'Expected missing file error');
    }
  ...
  (() => {
    ...
    try {
      ...
      threw = true;
      assert(/Cannot access archive directory/.test(err.message), 'Expected archive directory error');
    }
