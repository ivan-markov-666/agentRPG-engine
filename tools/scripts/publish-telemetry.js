...
function getArchiveFiles(dir, copyAll) {
  ...
  return [files.map((name) => ({ name, stat: fs.statSync(path.join(dir, name)) }))
    .sort((a, b) => b.stat.mtimeMs - a.stat.mtimeMs)[0]
    .name]
    .map((name) => path.join(dir, name));
}

function copyFile(src, destDir, dryRun) {
...
module.exports = { parseArgs, getArchiveFiles };
