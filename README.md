...
- **Exploration log schema:** `player-data/runtime/exploration-log.json` минава през `EXPLORATION-SCHEMA` guardrail (slug `id`, тип от {`city`,`landmark`,`dungeon`,`mcp`,`side-quest-hook`,`poi`}, `added_at` ISO8601, `origin` = `player-request`/`gm-suggested`, описания ≥40 символа, `tags` ≤10 уникални записа). Липсващ/празен лог при включено exploration води до `FILE-MISSING-OPTIONAL` / `EXPLORATION-EMPTY`.
