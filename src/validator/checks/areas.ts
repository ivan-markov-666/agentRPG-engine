...
import { add } from '../utils/io';
import type { BasicContext } from '../context';

const questContentCache = new Map<string, string | null>();

function clearQuestCache() {
  questContentCache.clear();
}

function getQuestContent(base: string, questId: string): string | null {
...
export async function checkAreas(ctx: BasicContext): Promise<void> {
  ...
  const entries = fs.readdirSync(areasDir).filter((file) => file.endsWith('.md'));
  if (!entries.length) {
  ...
  entries.forEach((file) => {
    ...
    linkMatches.forEach((target) => {
      ...
      if (fs.existsSync(targetQuestPath)) {
        ...
        if (questContent && !backlinkPattern.test(questContent)) {
          add(
            ...
            'AREA-QUEST-BACKLINK',
            relPath,
            `Quest [[${target}]] does not mention area '${areaId}'`,
            'Add [[area_id]] inside quest file or remove quest link from area',
          );
        }
        return;
      }
      ...
      add(
        ...
        'AREA-LINK',
        relPath,
        `Link [[${target}]] not found as quest or area`,
